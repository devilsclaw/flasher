...
{ 0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,'C' ,'O' ,'R' ,'E' ,0x20,0x20,0x20,0x20,
  'R' ,'E' ,'V' ,0x20,'L' ,'V' ,'L' ,0x20,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  'R' ,'O' ,'M' ,0x20,'R' ,'E' ,'V' ,0x20,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF
};

char main_pat[] =
{ 0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,'M' ,'A' ,'I' ,'N' ,0x20,0x20,0x20,0x20,
  'R' ,'E' ,'V' ,0x20,'L' ,'V' ,'L' ,0x20,0xFF,0xFF,0xFF,0xFF,0x00,0x00,0x00,0x00,
  'R' ,'O' ,'M' ,0x20,'R' ,'E' ,'V' ,0x20,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
  0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF
};
...
char get_firminfo_drive(int device,char* pattern,size_t patsize,firminfo* fi){
  ...
  if(!fi->firm_size){
    ...
    d.datasize = 8;
    d.data = tdata;
    if(!lgren_read(device,LOC_MEMORY,&d,swap32(fi->firm_start)+0xA0,d.datasize)){
      printf("get_firminfo_drive: failed to get firmware info 1\n");
      return 0;
    }

    if(((int*)d.data)[0] != fi->firm_start){
      if(!lgren_read(device,LOC_MEMORY,&d,swap32(fi->firm_start)+0x5A0,d.datasize)){
    ...
    if(((int*)d.data)[0] != fi->firm_start){
      printf("get_firminfo_drive: original firmware location and new different\n");
      return 0;
    }
...
char get_firminfo_buff(char* buff,char* pattern,size_t bsize,size_t patsize,firminfo* fi){
  ...
  if(!fi->firm_size){
    ...
    if(((int*)tdata)[0] != fi->firm_start){
      printf("get_firminfo_file: original firmware location and new different\n");
      return 0;
    }
...
unsigned short firm_chksum_calc(char* buff,size_t size,short key){
  ...
  for(count = 1;count < (size/2);count++){
    ...
    chksum += ((buff2[count] & 0x000000FF) << 8) | ((buff2[count] & 0x0000FF00) >> 8);
  }
  ...
  for(loop = 0; loop < 4;loop++){
    if(drive_ready(device)){
      if(!drive_safe_eject(device)){
        printf("firm_flasher: Please remove cd from drive and then try again.\n");
        return 0;
      }
    }
  }
...
size_t firm_dumper(int device,char** inbuff,int loc,size_t pos,size_t dsize){
  ...
  *inbuff = 0;
  ...
  while(start_pos < stop_pos)
  {
    ...
    size = ((SECTOR_SIZE + start_pos < stop_pos)?SECTOR_SIZE:SECTOR_SIZE - ((SECTOR_SIZE + start_pos) - stop_pos));
...
char firm_spliter(char* buff,size_t max_size,const char* outfile){
  ...
  do{
    ...
    if(firm_chksum_calc(&buff[fpos],firm_size,swap16(*(short*)(&buff[fpos])))){
      printf("firm_spliter: checksum failed for %s. but was still written to drive anyway.\n",ofile);
    }
    fpos += firm_size;
    loop++;
    free(ofile);
...
size_t find_pattern_drive(int device,int loc,char* pattern,size_t psize){
  ...
  found = 0;
  for(bcount = 0;bcount < (unsigned int)(0xFFFFFFFF);bcount+=SECTOR_SIZE - psize){
    ...
    for(scount = 0;scount < SECTOR_SIZE;scount++){
      for(pcount = 0;pcount < psize;pcount++){
        ...
        if(pattern[pcount] == d.data[scount+pcount] || pattern[pcount] == (char)0xFF){
          found = 1;
        }else{
          found = 0;
          break;
...
char firm_ripexe(char* filename){
  int type,found,firm_start,firm_stop,firm_end,
  dsize,firm_size,fpos;
  size_t fsize = 0 , key;
